...
function MindMapCanvas({ nodes, edges, workspaceId }: MindMapCanvasProps) {
  ...
  const placements = useMemo(
    () =>
      tree ? computeTreeLayout(tree, collapsed) : new Map<string, Placement>(),
    [tree, collapsed],
  );
  ...
  const flowEdges: Edge[] = useMemo(
    () =>
      edges
        ...
        .map((edge) => {
          const direction = placements.get(edge.target)?.direction ?? 1;
          const isSelected =
      ...
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/60 px-3 py-2">
          <p className="text-xs text-muted-foreground">
            Use the arrows to expand or collapse branches. Select a node to
            explore it in chat.
          </p>
